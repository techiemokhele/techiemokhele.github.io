<?php 
	...
	class Comment {
		...
		public function create() {
			...
			$timespan = $this->time_elasped_string($this->sqlData["datePosted"]); 
			...
			if ($numResponses > 0) {
				$viewRepliesText = "<span class='repliesSection viewReplies' onclick='getReplies($id, this, $videoId)'>
					<i class='fas fa-comment'></i> View all $numResponses replies</span>";
			} else {
		...
		public function getNumberOfReplies() {
			...
			/*Gets first(1) column found in table: comments*/
			return $query->fetchColumn();
		}

		function time_elasped_string($datetime, $full = false) {
			$now = new DateTime;
		...
		public function getLikes() {
			...
			/*Fetch all dislikes in a comment*/
